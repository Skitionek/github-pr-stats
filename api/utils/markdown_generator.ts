...
export class MarkdownGenerator {
  ...
  private static readonly REPOFIELD_CONFIGS: FieldsConfig<RepoFieldKey, RepoAggregate> = {
    ...
    pr_numbers: {
      label: 'PR Numbers',
      align: 'left',
      format: repo => MarkdownGenerator.escapeMarkdownCell(repo.pr_numbers.map(
  ...
  static generate (
    ...
    if (params.stats == 'all' || !params.stats) {
      ...
      summaryFields = selectedKeys.reduce<Partial<FieldsConfig<StatsFieldKey, PRStats>>>((acc, key) => {
